...
/**
 *  Check if any .eslintrc.* config exists in the project tree.
 *  Result is cached per project root. Call resetConfigCache() to invalidate.
 */
function hasConfig(filepath) {
  ...
  let found = false;
  for (const name of ESLINTRC_NAMES) {
    if (findFile(filepath, name)) {
      found = true;
      break;
    }
  }
  configCache.set(key, found);
  return found;
}
...
Object.assign(module.exports, { lint, triggerReLint, resetConfigCache });
