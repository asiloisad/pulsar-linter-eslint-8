...
function findFile(directory, name) {
  let dir = _m.path.dirname(directory);
  while (dir) {
    const filePath = _m.path.join(dir, name);
    try {
      _m.fs.accessSync(filePath, _m.fs.constants.R_OK);
      return filePath;
    } catch (_) {
      // File not found, continue upward
    }
    const parent = _m.path.dirname(dir);
    if (parent === dir) break;
    dir = parent;
  }
  return null;
...
function reLintOnTimer(grammar) {
  ...
  //
  // If you have any idea, please comment https://github.com/asiloisad/pulsar-linter-eslint/issues/4
...
function isIgnored(ignorefile, filepath) {
  const patterns = _m.fs
    .readFileSync(ignorefile, _STRINGS.UTF8)
    .replace(/^\ufeff/u, _STRINGS.EMPTY)
    .split(/\r?\n/gu)
    .filter(
      (line) =>
        line.trim() !== _STRINGS.EMPTY && !line.startsWith(_STRINGS.HASH)
    );
  const ignorepattern = new IgnorePattern(
    patterns,
    _m.path.dirname(ignorefile)
  );
...
function lint(texteditor) {
  ...
  if (atom.workspace.getTextEditors().includes(texteditor)) {
    ...
    const ignoreFile = findFile(filepath, _STRINGS.ESLINTIGNORE);
